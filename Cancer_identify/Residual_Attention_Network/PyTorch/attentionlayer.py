...
class AttentionModule_stage1(nn.Module):
    ...
    def forward(self, x):
        ...
        out_interp3 = self.upsample3(out_softmask3) #(6,6)->(12,12)
        out = out_interp3 + skip_connection2
        out_softmask4 = self.Resblock5(out)

        out_interp2 = self.upsample2(out_softmask4) #(12,12)->(24,24)
        out = out_interp2 + skip_connection1
        out_softmask5 = self.Resblock6(out)

        out_interp1 = self.upsample1(out_softmask5) #(24,24)->(48,48)
...
class AttentionModule_stage2(nn.Module):
    ...
    def forward(self, x):
        ...
        out_interp2 = self.upsample2(out_softmask2) #(6,6) ->(12,12)
        out = out_interp2 + skip_connection1
        out_softmask3 = self.Resblock4(out)
        out_interp1 = self.upsample1(out_softmask3) #(24,24)
...
class AttentionModule_stage3(nn.Module):
    ...
    def forward(self, x):
        ...
        out_softmask1 = self.Resblock2(pool1)
        out_interp1 = self.upsample1(out_softmask1) #(6,6) ->(12,12)
        out_softmask4 = self.output_block(out_interp1)
        out = (1 + out_softmask4) * out_trunk
        last_out = self.last_block(out)
